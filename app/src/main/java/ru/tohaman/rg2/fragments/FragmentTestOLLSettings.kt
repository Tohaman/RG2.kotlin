...
class FragmentTestOLLSettings : Fragment() {

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return TestOllUI<Fragment>().createView(AnkoContext.create(ctx, this))
    }

    companion object {
        fun newInstance(): FragmentTestOLLSettings {
            Log.v(DebugTag.TAG, "FragmentTestPLLSettings newInstance")
            return FragmentTestOLLSettings()
        }
    }
...
class TestOllUI<in Fragment> : AnkoComponentEx<Fragment>() {

    override fun create(ui: AnkoContext<Fragment>): View = with(ui) {
        Log.v(DebugTag.TAG, "TimerSettingsUI create start")
        val sp = PreferenceManager.getDefaultSharedPreferences(context)
        var rowCount = sp.getInt(OLL_TEST_ROW_COUNT, 6)
        var is3side = sp.getBoolean(OLL_TEST_3SIDE, true)

        linearLayout {
            gravity = Gravity.CENTER
            orientation = LinearLayout.VERTICAL

            include<View>(R.layout.fragment_oll_test_settings) {
